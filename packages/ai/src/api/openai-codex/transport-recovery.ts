import type { Api, AssistantMessage, Context, Model, Transport } from "../../types.ts";
import { appendAssistantMessageDiagnostic, createAssistantMessageDiagnostic } from "../../utils/diagnostics.ts";
import { AssistantMessageEventStream } from "../../utils/event-stream.ts";
import { createGrammarToolInputProperties } from "../constrained-sampling.ts";
import { withRemoteCompactionV2Feature } from "./compaction-v2-feature.ts";
import {
...
export function getEffectiveCodexTransport(
	...
	const preferredTransport =
		forceCachedWebSockets !== false && configuredTransport === "websocket" ? "websocket-cached" : configuredTransport;
...
export function createCodexTransportStream<TApi extends Api>(
	...
	const responsesLite =
		...
		((options?.executionMode === "code" || options?.executionMode === "notebook") &&
			supportsResponsesLiteModel(model));
	const grammarToolInputProperties = createGrammarToolInputProperties(context.tools, responsesLite);
	const preferredTransport = getEffectiveCodexTransport(options?.transport, options?.forceCachedWebSockets);
	const effectiveTransport = getEffectiveCodexTransport(
		options?.transport,
		options?.forceCachedWebSockets,
		options?.sessionId,
	);
	...
	(async () => {
		...
		try {
			...
			effectiveOptions?.onPreparedPayload?.(body);
			const websocketRequestId = effectiveOptions?.sessionId || createCodexRequestId();
			const routing = resolveCodexRequestRouting({
				model: body.model,
				fast: effectiveOptions?.fast === true,
				serviceTier: body.service_tier,
				normalOriginator: effectiveOptions?.harnessIdentifierHeader ? PI_CODEX_CONVERSION_ORIGINATOR : "pi",
			});
				...
				for (let attempt = 0; attempt <= streamMaxRetries; attempt++) {
					...
					try {
						await processWebSocketStream(
							resolveCodexWebSocketUrl(model.baseUrl),
							withCodexTurnState(websocketBody, effectiveOptions?.turnState),
							websocketHeaders,
							output,
							stream,
							model,
						...
						recordWebSocketFailure(effectiveOptions?.sessionId, error);
			...
			effectiveOptions?.onStreamSettled?.();
		}
	})();
