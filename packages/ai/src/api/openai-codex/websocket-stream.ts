...
import { type CanonicalSessionToken, recordCanonicalSessionResponse } from "./session-continuity.ts";
...
export async function processWebSocketStream<TApi extends Api>(
	...
	recordWebSocketRequest(options?.sessionId, {
		reused,
		cached: useCachedContext,
		store: requestBody.store,
		inputItems: requestBody.input.length,
		...(requestBody.previous_response_id ? { previousResponseId: requestBody.previous_response_id } : {}),
	});
