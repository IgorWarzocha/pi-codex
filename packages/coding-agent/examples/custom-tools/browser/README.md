...
```sh
mkdir -p ~/.pi/agent/custom-tools/browser
cp browser.toml ~/.pi/agent/custom-tools/
cp browser/*.mjs ~/.pi/agent/custom-tools/browser/
```

When `PI_CODING_AGENT_DIR` is configured, use `$PI_CODING_AGENT_DIR/custom-tools/` instead. Reload Pi after installing it.
