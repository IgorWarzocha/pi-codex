...
These are disabled templates. Enable a tool by copying its top-level TOML file and companion directory into `~/.pi/agent/custom-tools/`, or `$PI_CODING_AGENT_DIR/custom-tools/` when configured.
