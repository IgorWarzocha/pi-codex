...
let resetCreditsCache:
	| { key: string; expiresAt: number; promise: Promise<CodexRateLimitResetCredits | undefined> }
	| undefined;
...
function usageCacheKey(headers: Headers): string | undefined {
	...
	return authorization ? `auth:${createHash("sha256").update(authorization).digest("hex")}` : undefined;
}
