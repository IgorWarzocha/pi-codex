...
export function summarizeReadCommand(
	...
	if (head === "cat") return readOrUnknown(mainCommand, singleNonFlagOperand(tail, []));
	...
	if (head === "more") return readOrUnknown(mainCommand, singleNonFlagOperand(tail, []));
	if (head === "head" || head === "tail") return readOrUnknown(mainCommand, readPathFromHeadTail(tail, head));
	if (head === "awk") return readOrUnknown(mainCommand, awkDataFileOperand(tail));
	if (head === "nl") {
