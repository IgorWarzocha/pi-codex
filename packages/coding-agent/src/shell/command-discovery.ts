...
import { listCommand, type ParsedShellCommand, searchCommand, unknownCommand } from "./parsed-command.ts";
...
export function summarizeDiscoveryCommand(
	...
	if (head === "tree") {
		return listCommand(
			mainCommand,
			firstNonFlagOperand(tail, ["-L", "-P", "-I", "--charset", "--filelimit", "--sort"]),
		);
	}
	if (head === "du") {
		return listCommand(
			mainCommand,
			firstNonFlagOperand(tail, ["-d", "--max-depth", "-B", "--block-size", "--exclude", "--time-style"]),
		);
	}
	...
	if (head === "git") {
		...
		if (subcommand === "ls-files") {
			return listCommand(
				mainCommand,
				firstNonFlagOperand(subtail, ["--exclude", "--exclude-from", "--pathspec-from-file"]),
			);
		}
