...
import { type ParsedShellCommand, pathlessListCommand, unknownCommand } from "./parsed-command.ts";
...
export function summarizeMainTokens(mainCommand: string[]): ParsedShellCommand {
	...
	if (isPythonCommand(head)) {
		return pythonWalksFiles(tail) ? pathlessListCommand(mainCommand) : unknownCommand(mainCommand);
	}
