...
async function createBashParser(): Promise<Parser | undefined> {
	try {
		await Parser.init();
		const sourceAsset = fileURLToPath(new URL("./tree-sitter-bash.wasm", import.meta.url));
		const installedAsset = join(dirname(process.execPath), "shell", "tree-sitter-bash.wasm");
		const grammarPath = [sourceAsset, installedAsset].find(existsSync);
