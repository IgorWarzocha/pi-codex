...
const RECOVERY = "Reinstall Pi-Codex with native helpers built for this system";
...
export function nativeBinaryRecoveryMessage(
	...
	const startupPipeFailure =
		options.startupWriteFailure === true && (errorCode(error) === "EPIPE" || /\bEPIPE\b|broken pipe/i.test(message));
	const missingInterpreter = errorCode(error) === "ENOENT" && !!options.binaryPath && existsSync(options.binaryPath);
