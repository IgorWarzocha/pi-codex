...
export function mergeProjectState(options: {
	...
	return {
		...
		baseline: {
			...
			entries: [
				...skippedBaseline,
				...[...candidate.values()].map(({ name, hash, description, usage }) => ({
					name,
					hash,
...
export function projectStateEntryFingerprint(
	entry:
		| {
...
function projectStateEntryShape(
	entry: ProjectStateEntry,
): [string, string, string, boolean, string | null, string | null] {
