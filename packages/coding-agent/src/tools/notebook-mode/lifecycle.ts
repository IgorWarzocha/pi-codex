...
export class NotebookLifecycleController {
	...
	async control(
		...
		switch (request.action) {
			...
			case "pin":
				return this.pin(request.names, true);
			case "unpin":
				return this.pin(request.names, false);
			case "release":
				return this.release(request.names, context, signal);
			case "prune":
				return this.prune(request.query, context, signal);
			case "restart":
				return this.restart(context, signal);
		}
	}
	...
	private async status(query: string | undefined, signal?: AbortSignal): Promise<NotebookControlResult> {
		...
		const unpinned = retained.filter(({ pinned }) => !pinned).sort((left, right) => right.bytes - left.bytes);
