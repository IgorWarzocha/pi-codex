...
import { createJupyterConnectionFile, type JupyterConnectionInfo, jupyterEndpoint } from "./jupyter-connection.ts";
import {
...
export class DenoJupyterKernel {
	...
	async start(signal?: AbortSignal): Promise<void> {
		if (this.terminalFailure) {
			throw new Error(`Deno Jupyter kernel is unavailable: ${this.terminalFailure.message}`, {
	...
	async execute(
		...
		try {
			...
			const replyState = this.sendShellRequest(message, undefined, "execute_request").then((reply) => ({
				kind: "reply" as const,
				reply,
			}));
	...
	async complete(code = "", cursorPosition = code.length, signal?: AbortSignal): Promise<string[]> {
		...
		return Array.isArray(matches) ? matches.filter((value): value is string => typeof value === "string") : [];
	}
