...
export async function saveNotebookProfile(options: {
	...
	try {
		...
		const capture = await options.kernel.execute(
	...
	return withProfileLock(
		...
		async () => {
			...
			const current = new Set(
				[...new Set(await options.kernel.complete("", 0, options.signal))].filter(
					(name) => IDENTIFIER.test(name) && !options.baselineNames.has(name),
				),
			);
			...
			let restored: Awaited<ReturnType<DenoJupyterKernel["execute"]>>;
			try {
				restored = await options.kernel.execute(projectStateRestoreSource(manifest, payloadPath), {
					signal: options.signal,
				});
			} catch (error) {
			...
			if (restored.status !== "ok")
				throw new NotebookProfileRestoreError(
					`Notebook profile could not be loaded: ${restored.errorText ?? "unknown error"}`,
				);
			return {
