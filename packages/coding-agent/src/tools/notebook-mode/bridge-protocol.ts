...
export function writeNotebookBridgeJson(response: ServerResponse, status: number, value: unknown): void {
	...
	try {
		...
		body = JSON.stringify({
			ok: false,
			error: `Notebook bridge result is not serializable: ${error instanceof Error ? error.message : String(error)}`,
		});
	}
	...
	return value.map((item) => {
		...
		if (item["type"] === "input_image" && typeof item["image_url"] === "string") {
			...
			return {
				...
				...(detail === "auto" || detail === "low" || detail === "high" || detail === "original" || detail === null
					? { detail }
					: {}),
			};
		}
