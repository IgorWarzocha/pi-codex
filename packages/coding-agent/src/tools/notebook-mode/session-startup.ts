...
import { notebookSessionIdentity } from "../code-mode/notebook-session.ts";
import type { NotebookRuntimeOptions } from "../code-mode/runtime.ts";
...
import { formatProjectStateNotice, type ProjectStateBaseline, restoreProjectState } from "./project-state.ts";
...
async function installNotebookExamples(kernel: DenoJupyterKernel, signal?: AbortSignal): Promise<string[]> {
	...
	try {
		...
		const result = await kernel.execute(notebookExampleSource(marker, names.has("foo") || names.has("bar")), {
			signal,
		});
		...
		return Array.isArray(value) && value.every((name) => name === "foo" || name === "bar") ? [...new Set(value)] : [];
	} catch {
		signal?.throwIfAborted();
		return [];
	}
