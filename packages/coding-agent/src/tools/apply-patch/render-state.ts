...
export function isApplyPatchToolDetails(details: unknown): details is ApplyPatchToolDetails {
	...
	if (
		...
		return false;
	return record.status === "success" || record.failedTargets === undefined || isStringArray(record.failedTargets);
}
...
function renderFailedCall(
	...
	return lines
		.map((line, index) => (failedLineIndexes.has(index) || index === 0 ? theme.fg("error", line) : line))
		.join("\n");
}
