export const APPLY_PATCH_DISPLAY_PROTOCOL = "pi-codex/apply-patch-display/v1";
...
export function isApplyPatchDisplayRequest(value: unknown): boolean {
	return Boolean(
		value && typeof value === "object" && "protocol" in value && value.protocol === APPLY_PATCH_DISPLAY_PROTOCOL,
	);
}

export function isApplyPatchDisplayBroker(value: unknown): value is ApplyPatchDisplayBroker {
	return Boolean(
		value &&
			typeof value === "object" &&
			"protocol" in value &&
