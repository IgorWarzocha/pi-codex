...
const APPLY_PATCH_PARAMETERS = Type.Object({
	input: Type.String({
		description:
			"Full patch text. Use *** Begin Patch / *** End Patch with Add/Update/Delete File sections. *** Move to: path must immediately follow its Update File header and still needs a nonempty @@ hunk; use one unchanged context line for a pure move. Order each file's hunks top-to-bottom; indentation is literal",
	}),
});
...
function touchedPatchPaths(cwd: string, patchText: string): string[] {
	try {
		...
		return [
			...new Set(
				paths.filter((path): path is string => !!path).map((patchPath) => resolvePatchPath({ cwd, patchPath })),
			),
		].sort();
	} catch {
...
const renderApplyPatchCallWithOptionalContext = (
	...
	new Text(
		renderApplyPatchCallFromState(args, theme, { ...context, showCollapsedDiff: options.showDiffWhenCollapsed }),
		0,
		0,
	);
...
export function createApplyPatchTool(options: ApplyPatchToolOptions = {}): ApplyPatchToolDefinition {
	...
	const compactRendering = (context?: ApplyPatchRenderContextLike) =>
		shouldCompactApplyPatchDisplay(context?.toolCallId, context?.executionStarted);
	const defaultRenderCall: ApplyPatchRenderCall = (args, theme, context) =>
		compactRendering(context)
