...
export function shouldCompactApplyPatchDisplay(toolCallId?: string, executionStarted?: boolean): boolean {
...
export function recordApplyPatchDisplayOutcome(toolCallId: string, outcome: CapturedApplyPatchOutcome): void {
...
export function registerApplyPatchDisplayBroker(pi: ExtensionAPI): void {
	...
	pi.on("tool_execution_start", (event) => {
		if (event.toolName === "apply_patch" && active && registrations.size > 0) activeCalls.add(event.toolCallId);
	});
...
function collectApplyPatchDisplayData(
	...
	return nestedTraces(event.details).flatMap((trace) => {
		if (trace.name !== "apply_patch" || trace.status === "running" || typeof trace.id !== "string") return [];
