...
export function buildWebSearchInput(entries: SessionEntry[]): SearchInputMessage[] | undefined {
	const userEntryIndexes = entries.flatMap((entry, index) =>
		entry.type === "message" && entry.message.role === "user" && visibleText(entry.message).length > 0 ? [index] : [],
	);
	...
	const input = entries.slice(startIndex, currentUserIndex + 1).flatMap((entry) => {
