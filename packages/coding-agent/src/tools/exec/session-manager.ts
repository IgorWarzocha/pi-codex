import { type BridgeExecSession, type BridgeSessionHooks, createBridgeSessionRuntime } from "./bridge-session.ts";
...
export function createExecSessionManager(options: ExecSessionManagerOptions = {}): ExecSessionManager {
	...
	const configuredMaxSessionBufferChars =
		options.maxSessionBufferChars === undefined ? undefined : Math.max(1024, options.maxSessionBufferChars);
	...
	function replayCompletedResult(result: UnifiedExecResult, maxOutputTokens?: number): UnifiedExecResult {
		const originalCharCount =
			result.original_token_count === undefined ? result.output.length : result.original_token_count * 4;
	...
	function finishResult(session: ExecSession, waitMs: number, maxOutputTokens?: number): UnifiedExecResult {
		...
		const replaySnapshot = completed
			? makeSnapshotResult(session, waitMs, MAX_COMPLETED_SESSION_OUTPUT_TOKENS, true)
			: undefined;
		...
		exec: async (input, cwd, signal, onUpdate) => {
			...
			try {
				...
				const waitedMs = await waitForExitOrInactivity(
					...
					signal,
					onUpdate
						? (elapsedMs) => onUpdate(makeSnapshotResult(session, elapsedMs, input.max_output_tokens))
						: undefined,
				);
			...
			const waitedMs =
				session.exitCode === undefined
					? await waitForExitOrInactivity(
							...
							onUpdate
								? (elapsedMs) =>
										onUpdate(makeSnapshotSince(session, elapsedMs, updateBaseline, input.max_output_tokens))
								: undefined,
						)
					: 0;
			await bridgeSessions.waitForStartup(session, signal);
