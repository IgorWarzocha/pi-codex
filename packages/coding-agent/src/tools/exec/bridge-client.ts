...
export function formatExecBridgeExitError(
	stderr: string,
...
function formatExecBridgeWriteError(error: Error, stderr: string, startupWriteFailure: boolean): string {
	const detail = stderr.trim();
	return formatNativeBinaryError("exec_bridge", detail ? `${error.message}: ${detail}` : error, {
		startupWriteFailure,
	});
}

export function createExecBridgeClient(
	binaryPath: () => string | undefined = () => getBundledToolBinaryPath("exec_bridge"),
): ExecBridgeClient {
	let bridge: ChildProcessWithoutNullStreams | undefined;
	let nextBridgeRequestId = 1;
	const pendingBridgeRequests = new Map<
		number,
		{ resolve: (value: BridgeResponse) => void; reject: (error: Error) => void }
	>();
	...
	function getBridge(): ChildProcessWithoutNullStreams {
		...
		bridge.on("close", (code, signal) => {
			...
			rejectPending(
				new Error(bridgeClosing ? "exec_bridge closed" : formatExecBridgeExitError(bridgeStderr, code, signal)),
			);
			bridge = undefined;
			bridgeLineBuffer = "";
			bridgeStderr = "";
		...
		bridge.on("error", (error) =>
			rejectPending(new Error(formatNativeBinaryError("exec_bridge", error, { binaryPath: binary }))),
		);
		return bridge;
	}
	...
	return {
		request,
		shutdown: () => {
			if (!bridgeShutdownPromise) bridgeShutdownPromise = shutdownBridge();
...
function killUnixDescendants(rootPid: number): void {
	...
	const processes = result.stdout.split("\n").flatMap((line) => {
		...
		return pid && parentPid !== undefined && processGroupId ? [{ pid, parentPid, processGroupId }] : [];
	});
...
function waitForBridgeClose(child: ChildProcessWithoutNullStreams, timeoutMs: number): Promise<void> {
	...
	return withTimeout(
		...
		timeoutMs,
		"exec_bridge did not exit after shutdown",
	);
}
