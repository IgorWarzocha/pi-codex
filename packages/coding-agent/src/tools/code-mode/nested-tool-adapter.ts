...
import type { CodeModeToolIdentity, ProgrammaticCodeModeToolDefinition, ToolExecutionContext } from "./types.ts";
...
export function toNestedTool<TParams extends TSchema, TDetails, TState>(
	...
	const prepareInput = (input: unknown) => (contract.prepareInput ? contract.prepareInput(input) : input);
	return {
		name: tool.name,
		usage,
...
export function codeModeImageResult(result: AgentToolResult<unknown>, outputHint?: string): unknown {
	const image = result.content.find((item) => item.type === "image");
	if (!image || image.type !== "image") return compactNestedResult(result);
	const detail = "detail" in image && typeof image.detail === "string" ? image.detail : "high";
	return {
...
function forwardUpdate(update: AgentToolResult<unknown>, context: ToolExecutionContext): void {
