...
export const CUSTOM_TOOLS_DIRNAME = "custom-tools";
...
export function getProjectCustomToolsDir(launchDir: string = process.cwd()): string {
...
function optionalNonNegativeInteger(value: unknown, field: string, path: string): number | undefined {
	if (value === undefined) return undefined;
...
export function parseCustomTool(path: string, text: string): CustomToolDefinition {
...
export function discoverCustomToolsFromDirectories(dirs: readonly string[]): CustomToolDiscoveryResult {
...
function customToolPaths(dir: string, errors: CustomToolDiscoveryError[]): string[] {
	try {
...
function loadCustomTool(path: string, errors: CustomToolDiscoveryError[]): CustomToolDefinition | undefined {
	try {
		return parseCustomTool(path, readFileSync(path, "utf8"));
	} catch (error) {
		const detail = error instanceof Error ? error.message : String(error);
		const message = detail.startsWith(`${path}:`) ? detail : `${path}: ${detail}`;
		const name = basename(path, extname(path));
