...
import type { CodeModeToolDefinition, CustomToolDefinition, RuntimeContentItem, RuntimeResponse } from "./types.ts";
...
export function toWireToolDefinition(tool: CodeModeToolDefinition) {
	...
	return {
		...
		input_schema: isCustomToolDefinition(tool) || tool.kind === "freeform" ? null : (tool.inputSchema ?? null),
		output_schema: null,
	};
}
...
function parseContentItems(value: unknown): RuntimeContentItem[] {
	...
	return value.map((item) => {
		...
		if (item.type === "input_text" && typeof item.text === "string") return { type: "input_text", text: item.text };
		...
		if (item.type === "input_audio") throw new Error("Code-mode audio output is not supported by Pi");
