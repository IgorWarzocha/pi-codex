...
export function scopeAllToolsToDeferredCustom(source: string, tools: CodeModeToolDefinition[]): string {
	const names = tools
...
function hasExecutableBracketReference(code: string, executableCode: string, pattern: RegExp): boolean {
	for (const match of code.matchAll(pattern)) {
		if (match.index !== undefined && executableCode.slice(match.index, match.index + 5) === "tools") return true;
	}
	return false;
}
