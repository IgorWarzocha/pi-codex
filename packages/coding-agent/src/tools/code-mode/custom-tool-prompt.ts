import type { CodeModeToolDefinition, CodeModeToolMetadata, CustomToolDefinition } from "./types.ts";
...
export function formatCodeModeToolHelp(tool: CodeModeToolMetadata): string {
	return [`Usage: ${tool.usage}`, tool.description, tool.output ? `Output: ${tool.output}` : undefined]
		.filter(Boolean)
		.join("\n");
}
...
export function buildCodeModeToolsPrompt(tools: CodeModeToolDefinition[], existingPrompt = ""): string {
	...
	const sections = [
		existingPrompt.includes(BUNDLED_TOOLS_HEADING) ? undefined : buildUsageSection(BUNDLED_TOOLS_HEADING, bundled),
