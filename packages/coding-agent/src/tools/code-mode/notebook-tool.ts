...
import type { CodeModeRuntime } from "./runtime.ts";
import type {
...
const NOTEBOOK_DESCRIPTION =
	"Control persistent notebook state: status inspects memory/bindings by query glob; checkpoint; pin/unpin/release names; prune unpinned matches; list/save/load profiles; restart; diagnostics; reset";
...
export function createNotebookTool(runtime: CodeModeRuntime): ToolDefinition<typeof NOTEBOOK_PARAMETERS> {
	...
	return {
		...
		async execute(_id, params, signal, _onUpdate, ctx) {
			const result = await executeNotebookControl(runtime, params, { cwd: ctx.cwd, extensionContext: ctx }, signal);
			return {
...
export function createNotebookControlProxy(runtime: CodeModeRuntime): ProgrammaticCodeModeToolDefinition {
	return {
		name: "notebook",
...
export function executeNotebookControl(
	runtime: CodeModeRuntime,
	params: NotebookToolParameters,
	context: ToolExecutionContext,
	signal?: AbortSignal,
): Promise<NotebookControlResult> {
	return runtime.controlNotebook(normalizeNotebookRequest(params), context, signal);
}
