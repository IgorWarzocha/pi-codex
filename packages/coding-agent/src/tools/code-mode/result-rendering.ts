...
import { imagesByMimeType, previewText, type RenderedToolContent, renderTextAndImages } from "./render-content.ts";
...
function renderCodeModeResult(
	...
	const renderedContent =
		notebookMemoryText && content[0]?.type === "text" && content[0].text === notebookMemoryText
			? content.slice(1)
			: content;
	...
	const ratio =
		details.notebookMemory.heapLimitBytes > 0
			? details.notebookMemory.heapUsedBytes / details.notebookMemory.heapLimitBytes
			: 0;
	container.addChild(
		new Text(theme.fg(ratio >= 0.9 ? "error" : ratio >= 0.8 ? "accent" : "muted", notebookMemoryText), 0, 0),
	);
