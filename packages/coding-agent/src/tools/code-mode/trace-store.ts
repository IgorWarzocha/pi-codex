...
import type { RuntimeResponse, RuntimeToolResult, RuntimeToolTrace, ToolExecutionContext } from "./types.ts";
		...
		try {
			context.onUpdate?.({
				...
				details: {
					...
					...(this.droppedCounts.get(cellId) ? { droppedTraceCount: this.droppedCounts.get(cellId) } : {}),
				},
			});
		} catch {
