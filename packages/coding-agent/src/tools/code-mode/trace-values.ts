...
export function toolResultFromValue(value: unknown): RuntimeToolResult {
	return {
		content: [
			{
				type: "text",
				text: typeof value === "string" ? value : safeStringify(value, "(non-serializable tool result)"),
			},
		],
	};
}
...
export function boundRuntimeToolResult(result: RuntimeToolResult, imageCharsRemaining: number): RuntimeToolResult {
...
function sanitizeValue(value: unknown, budget: SerializationBudget): unknown {
	...
	if (typeof value === "string") {
		...
		return value.length <= available ? value : `${value.slice(0, Math.max(0, available - 21))}[value truncated]`;
	}
