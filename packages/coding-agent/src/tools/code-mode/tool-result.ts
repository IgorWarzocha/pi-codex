...
export function toCodeModeToolResult(response: RuntimeResponse, maxTokens?: number) {
	...
	return {
		content: [{ type: "text" as const, text: status }, ...truncateTextContent(output, outputTokens * 4)],
		details: {
			codeMode: true,
...
function numericSessionId(value: unknown): number | undefined {
	if (value && typeof value === "object" && "session_id" in value && typeof value.session_id === "number")
...
function truncateTextContent<T extends { type: string; text?: string }>(content: T[], maxChars: number): T[] {
