...
import { type DelegateRequestMessage, isCustomToolDefinition } from "./host-protocol.ts";
...
export class CodeModeDelegateRuntime {
	...
	handleRequest(message: DelegateRequestMessage): void {
		...
		if (this.controllers.has(key)) throw new Error(`Duplicate code-mode delegate request: ${message.id}`);
		const controller = new AbortController();
		const cellId =
			message.request.type === "notification/send" ? message.request.cellId : message.request.invocation.cell_id;
		this.controllers.set(key, { cellId, controller });
		void this.invoke(message, key, controller);
	}

	async invokeDirect(cellId: string, requestId: number, toolName: string, input: unknown): Promise<unknown> {
		const key = directControllerKey(cellId, requestId);
		if (this.controllers.has(key)) throw new Error(`Duplicate code-mode delegate request: ${requestId}`);
		const controller = new AbortController();
		this.controllers.set(key, { cellId, controller });
		try {
	...
	private async invoke(message: DelegateRequestMessage, key: string, controller: AbortController): Promise<void> {
	...
	private async invokeTool(
		...
		try {
			await runCodeModeToolPreflight(tool.name, input, invocationContext, controller.signal);
			...
			if (!trace.result) trace.result = this.traces.captureResult(cellId, trace, toolResultFromValue(result));
			trace.status = "done";
			this.traces.emitUpdate(cellId, context);
			return result;
		} catch (error) {
			trace.status = "error";
			trace.error = truncateTraceText(error instanceof Error ? error.message : String(error), MAX_TRACE_ERROR_CHARS);
			this.traces.emitUpdate(cellId, context);
			throw error;
		}
	}
