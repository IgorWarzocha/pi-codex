import type { CodexDiagnosticsFailure, CodexDiagnosticsFailureCategory } from "./types.ts";
...
export function codexDiagnosticsFailure(error: unknown): CodexDiagnosticsFailure {
	...
	const status = statusCode(outer?.status) ?? statusCode(payload?.status) ?? statusCode(responseError?.status);
	const description = [
