import type { EntryRenderer, ExtensionAPI } from "./core/extensions/types.ts";
import {
...
export function registerApplyPatchDisplay(
	...
	if (!customType) throw new Error("apply_patch display customType cannot be empty");
	...
	const unregisterAvailable = pi.events.on(APPLY_PATCH_DISPLAY_AVAILABLE_CHANNEL, (value) => {
		if (disposed || !isApplyPatchDisplayBroker(value) || value === broker) return;
		unregisterDisplay?.();
		broker = value;
