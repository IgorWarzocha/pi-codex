...
import { resolveContextPath } from "./paths.ts";
...
function gitCommandInfo(parts: string[], index: number) {
	...
	return { subcommand: parts[cursor]?.toLowerCase() ?? "", directory, subcommandIndex: cursor };
}
...
function isDiscoveryCommandAt(parts: string[], index: number): boolean {
	...
	return command === "git" && ["ls-files", "grep"].includes(gitCommandInfo(parts, index).subcommand);
}
...
export function isDiscoveryShellCommand(value: string): boolean {
	...
	return parts.some((_part, index) => isDiscoveryCommandAt(parts, index));
}

export function isPathOutputShellCommand(value: string): boolean {
	const parts = shellCommandParts(value);
	return parts.some((_part, index) => isPathOutputCommandAt(parts, index));
}
...
function maybePushPath(paths: string[], item: string, base: string): void {
	...
	if (item.startsWith("/")) {
		if (existsSync(item)) paths.push(resolveContextPath(item, base));
		return;
	}
	if (
		(item.startsWith("./") || item.startsWith("../") || item.includes("/")) &&
		existsSync(resolveContextPath(item, base))
	) {
		paths.push(resolveContextPath(item, base));
	}
}
...
export function shellTargets(value: string, base: string): string[] {
	...
	for (let index = 0; index < parts.length; index += 1) {
		...
		if (item === "cd") {
			...
			if (next) cwd = resolveContextPath(next, cwd);
			index += 1;
			scanningDiscoveryCommand = false;
			continue;
		}
		if (isDiscoveryCommandAt(parts, index)) {
			...
			if (item.toLowerCase() === "git") {
				...
				if (directory) {
					discoveryBase = resolveContextPath(directory, cwd);
		...
		if (!scanningDiscoveryCommand || item.startsWith("-") || item.includes("=")) continue;
...
export function shellOutputBase(value: string, base: string): string {
	...
	for (let index = 0; index < parts.length; index += 1) {
		...
		if (isDiscoveryCommandAt(parts, index)) {
			...
			return directory ? resolveContextPath(directory, cwd) : cwd;
		}
	}
	return cwd;
}
