...
import type { ContextEvent, ExtensionAPI, ExtensionContext } from "../core/extensions/types.ts";
import { type RealtimePeerPlan, startControllerMode, type VoiceControllerRuntime } from "./controller-start.ts";
import {
	currentVoiceSession,
...
export class CodexVoiceController {
	...
	constructor(pi: ExtensionAPI) {
		this.messages = new CodexVoiceSessionMessages(pi, {
			...
			onDelegation: (id) => {
				if (this.runtime.state.type === "conversation") this.runtime.state.session.activateDelegation(id);
			},
			onDelegationFailed: () => {
				if (this.runtime.state.type === "conversation") this.runtime.state.session.settleAgentTurn();
			},
	...
	setDelegationPreflight(
		preflight: (ctx: ExtensionContext, signal: AbortSignal) => Promise<PreparedVoiceDelegation | undefined>,
	): void {
	...
	setPrompt(report: { id: string; active: boolean; prompt: string }): void {
		...
		if (this.runtime.state.type === "conversation") this.runtime.state.session.announcePrompt(report.prompt);
	}
	announceCompactionStart(reason: "threshold" | "overflow"): void {
		if (this.runtime.state.type === "conversation") this.runtime.state.session.announceCompactionStart(reason);
	}
	...
	get inputMuted(): boolean {
		return this.runtime.state.type === "conversation" && this.runtime.state.session.microphoneMuted;
	}
	...
	setInputMuted(muted: boolean): boolean {
		if (this.runtime.state.type !== "conversation" || this.runtime.announcedMode !== "realtime") return false;
	...
	async stopRealtimeWithPeerPlan(plan: RealtimePeerPlan, options?: { announce?: boolean }): Promise<void> {
	...
	async stop(options?: { announce?: boolean }): Promise<void> {
		...
		const endedMode = options?.announce ? this.runtime.announcedMode : undefined;
		...
		if (this.runtime.startGeneration === stopGeneration) this.messages.voiceStopped(endedMode);
	}
	...
	mirrorPiSteer(input: unknown): boolean {
		return this.runtime.state.type === "conversation" && this.runtime.state.session.mirrorPiSteer(input);
	}

	streamDelta(delta: string): void {
		if (this.runtime.state.type === "conversation") this.runtime.state.session.streamAgentDelta(delta);
	}
	...
	finishAgentMessage(message: AssistantMessage, forwardReasoningSummaries: boolean): void {
		...
		const completedText = message.content.flatMap((part) => (part.type === "text" ? [part.text] : [])).join("\n");
		...
		if (channel === "commentary") {
			if (completedText.trim()) {
				...
				this.runtime.state.session.finishAgentMessage("speakable", completedVoiceReasoningSummary(message));
			}
			return;
		}
	...
	private fail(error: Error, failedSession?: CodexRealtimeConversation | undefined): void {
		if (this.runtime.state.type === "idle" || this.runtime.state.type === "failed") return;
		this.runtime.startAbortController?.abort();
		this.runtime.startAbortController = undefined;
		const mode = voiceModeForState(this.runtime.state);
		const message = this.runtime.config ? formatVoiceAudioError(error, mode, this.runtime.config) : error.message;
		const failGeneration = ++this.runtime.startGeneration;
		const endedMode = this.runtime.announcedMode;
		const wasMuted = this.inputMuted;
		const session = this.currentSession();
		if (failedSession) this.markRealtimePeerInactive(failedSession, error, false);
		const closePromise = session?.close();
	...
	private drop(session: CodexRealtimeConversation, error: Error): void {
		...
		void (async () => {
			await Promise.allSettled([session.close(), this.messages.waitForDelegations()]);
			if (this.runtime.startGeneration !== resumeGeneration || this.runtime.state.type !== "reconnecting") return;
			const resumePromise = this.startMode(ctx, config, "realtime", realtimePeerPlan, undefined, true, wasMuted);
			...
			try {
				...
				if (this.runtime.startGeneration === replacementGeneration)
					this.fail(resumeError instanceof Error ? resumeError : new Error(String(resumeError)));
				return;
			}
			if (!resumed) {
				const resumeError = new Error("Codex realtime voice could not resume");
				this.markRealtimePeerInactive(session, resumeError, false, realtimePeerPlan);
				if (this.runtime.state.type === "reconnecting") this.fail(resumeError);
				return;
			}
			if (resumed && wasMuted && this.currentSession() === resumed) this.renderCurrentStatus();
		})();
	}
