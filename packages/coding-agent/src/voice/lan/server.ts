...
export async function startCodexLanVoiceServer(options: {
		...
		const plan: RealtimePeerPlan = {
			...
			createPeer: () => {
				...
				peer = new LanHostRealtimePeer({
					onAudio: (pcm) => {
						if (activeConversation?.peer === peer) clients.sendConversationAudio(pcm);
					},
				});
				return peer;
			},
			...
			onInactive: (conversation, error, resuming) => {
				...
				if (activated) clients.broadcastControl({ type: "error", message: error.message });
			},
		};
		realtimePlan = plan;
		const promise = (async () => {
	...
	clients = new LanVoiceBrowserClients({
		...
		async onConversationActivity(active) {
			...
			if (active) {
				if (current) options.voice.setConversationInputActive(current.conversation, true);
				return;
			}
			const plan = realtimePlan;
			realtimePlan = undefined;
			activeConversation = undefined;
			if (plan) await options.voice.stopRealtimeWithPeerPlan(plan, { announce: true });
		},
	...
	const closeServer = async (): Promise<void> => {
		...
		activeConversation = undefined;
		if (remainingPlan)
			await collectFailures([options.voice.stopRealtimeWithPeerPlan(remainingPlan, { announce: true })], failures);
