...
import { type LanVoiceBrowserClientsOptions, LanVoiceBrowserSession } from "./browser-session.ts";
	...
	private receive(clientId: string, socket: WebSocket, data: RawData, isBinary: boolean): void {
		...
		try {
			...
			if (message.type === "start") {
				...
				void this.session
					.finish(clientId, socket, message.draft, message.revision, message.selection)
					.catch((error: unknown) => this.sendSocketError(socket, error));
