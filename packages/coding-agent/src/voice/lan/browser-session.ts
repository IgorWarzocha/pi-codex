...
import type { LanVoiceBrowserConnections } from "./browser-connections.ts";
...
export class LanVoiceBrowserSession {
	...
	release(clientId: string, socket?: WebSocket, terminateConversation = false): void {
		...
		void this.enqueue(async () => {
			const active = this.state;
			const ownsActive =
				active.type === "active" && active.clientId === clientId && (!socket || active.socket === socket);
			...
			this.connections.sendControl(clientId, { type: "error", message: errorMessage(error) }),
		);
	}
		...
		return this.enqueue(async () => {
			...
			socket.send(
				JSON.stringify({
					type: "active",
					mode,
					...(mode === "conversation" ? { muted: this.options.conversationMuted() } : {}),
				}),
			);
		});
	}
