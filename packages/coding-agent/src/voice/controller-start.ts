...
import { currentVoiceSession, VOICE_STATUS_KEY, type VoiceState } from "./controller-support.ts";
...
export async function startControllerMode(options: {
	...
	const realtimePrompt = options.mode === "realtime" ? options.prepareRealtimePrompt(options.ctx) : undefined;
	if (options.mode === "realtime" && realtimePrompt === undefined) return;
	if (!options.resume) {
		if (runtime.state.type === "dictation") await options.finishCurrentDictation();
	...
	const startSignal = signal ? AbortSignal.any([signal, startAbortController.signal]) : startAbortController.signal;
	...
	try {
		...
		if (startGeneration !== runtime.startGeneration || runtime.state.type !== "connecting") return;
		if (options.mode === "dictation") await startDictation(options, auth);
		else await startConversation(options, auth, realtimePrompt!, initialItems, startSignal);
...
async function startConversation(
	...
	await startControllerConversation({
		...
		greeting: options.resume ? undefined : initialItems?.length ? "contextual" : "fresh",
		peer,
		signal,
		lifecycle: {
			...
			onError: (session, error) => {
				if (currentVoiceSession(runtime.state) === session) options.onError(error, session);
			},
			onDrop: (session, error) => {
				if (currentVoiceSession(runtime.state) === session) options.onDrop(session, error);
			},
			onStatus: options.onStatus,
			onTurn: (turn) => {
				void options.messages.voiceTurn(turn);
			},
...
async function startDictation(
	...
	await startControllerDictation({
		...
		lifecycle: {
			...
			onError: (session, error) => {
				if (currentVoiceSession(runtime.state) === session) options.onError(error);
			},
			onStatus: options.onStatus,
...
function cancelStart(runtime: VoiceControllerRuntime, startGeneration: number): void {
	if (startGeneration !== runtime.startGeneration) return;
