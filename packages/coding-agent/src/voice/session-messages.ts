...
export class CodexVoiceSessionMessages {
	...
	userTranscript(transcript: string): void {
		this.pi.appendEntry<RealtimeUserTranscriptMessageDetails>(REALTIME_USER_TRANSCRIPT_MESSAGE_TYPE, { transcript });
	}
	...
	voiceTurn(turn: RealtimeVoiceTurn): Promise<void> {
		if (!turn.delegationId) {
			this.pi.appendEntry<RealtimeVoiceMessageDetails>(REALTIME_VOICE_MESSAGE_TYPE, {
				input: turn.input,
				route: "conversation",
			});
	...
	private appendMode(mode: CodexVoiceMode, state: CodexVoiceModeState): void {
		...
		this.pi.appendEntry<CodexVoiceModeMessageDetails>(CODEX_VOICE_MODE_MESSAGE_TYPE, { mode, state });
	}
	...
	private async deliverDelegation(turn: RealtimeVoiceTurn, generation: number, canDeliver: boolean): Promise<void> {
		const ctx = this.context;
		if (generation !== this.contextGeneration || !ctx || !turn.delegationId || !canDeliver) return;
		...
		try {
			...
			if (startsTurn) {
				for (;;) {
					...
					if (generation !== this.contextGeneration || this.context !== ctx) return;
			...
			this.pi.sendMessage(
				...
				startsTurn ? { triggerTurn: true } : { triggerTurn: true, deliverAs: "steer" },
			);
		} catch (error) {
			if (generation !== this.contextGeneration || this.context !== ctx) return;
			if (deliveryStarted) {
			...
			try {
				this.pi.appendEntry<RealtimeVoiceMessageDetails>(REALTIME_DELEGATION_MESSAGE_TYPE, {
					input: turn.input,
					route: "delegation",
					error: message,
				});
