...
import { DEFAULT_VOICE_CONTEXT_REASONING, type VoiceContextReasoning } from "../adapter/activation/config.ts";
...
import { isNativeCompactionEntry, type NativeCompactionEntry } from "../adapter/compaction/types.ts";
...
export async function createNativeVoiceContextSummary(request: NativeVoiceContextRequest): Promise<string> {
	const branch = request.ctx.sessionManager.getBranch();
	const checkpointIndex = findLatestCompactionEntryIndex(branch);
	if (checkpointIndex === undefined) throw new Error("Native checkpoint is missing from the active branch");
	...
	let completed: { content: Array<{ type: string; text?: string }>; errorMessage?: string } | undefined;
