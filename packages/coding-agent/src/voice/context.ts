import { type Api, type Context, type Model, uuidv7 } from "@earendil-works/pi-ai";
...
const SUMMARIZABLE_CUSTOM_TYPES = new Set([REALTIME_DELEGATION_MESSAGE_TYPE, "codex-realtime-voice-tail"]);
...
export async function buildRealtimeInitialItems(args: {
	...
	if (selected) {
		...
		if (!text) {
			...
			try {
				const generated = await createVoiceContextSummary(args.ctx, selected, reasoning, args.signal);
				if (generated) {
					text = generated;
...
function serializeVoiceConversation(messages: ReturnType<typeof buildSessionContext>["messages"]): string {
	const turns: string[] = [];
	for (const message of messages) {
		...
		if (message.role === "assistant") {
			if (message.stopReason !== "toolUse") appendTurn(turns, "Assistant", textContent(message.content));
			continue;
		}
		...
		if (message.role === "custom" && SUMMARIZABLE_CUSTOM_TYPES.has(message.customType))
...
function resolveSelectedModel(ctx: ExtensionContext, selected: VoiceContextModel): Model<Api> {
	const model = ctx.modelRegistry.find(selected.provider, selected.modelId);
	if (!model) throw new Error(`Voice context model is unavailable: ${selected.provider}/${selected.modelId}`);
	return model;
}
...
async function completeWithSelectedModel(
	...
	if (!provider) throw new Error(`Voice context provider is unavailable: ${model.provider}`);
	const auth = await ctx.modelRegistry.getApiKeyAndHeaders(model);
	if (!auth.ok) throw new Error(auth.error);
	const requestModel = auth.baseUrl ? { ...model, baseUrl: auth.baseUrl } : model;
	let completed: { content: Array<{ type: string; text?: string }> } | undefined;
	for await (const event of provider.streamSimple(requestModel, context, {
		...
		if (event.type === "error") throw new Error(event.error.errorMessage || "Voice context model failed");
	}
	return (
		completed?.content
...
function requireSummary(value: string): string {
	...
	if (!summary) throw new Error("Voice context model returned an empty summary");
	return summary;
}

function latestCompactionIsOpaque(ctx: ExtensionContext): boolean {
	const latest = [...ctx.sessionManager.getBranch()].reverse().find((entry) => entry.type === "compaction");
	...
	return (
		...
		"strategy" in details &&
		(details.strategy === "openai-responses-compaction-v2" || details.strategy === "openai-native-compact-v1")
	);
}
...
function voiceContextCacheKey(
	...
	const boundary = [...ctx.sessionManager.getBranch()]
		.reverse()
		.find(
			(entry) =>
				entry.type === "message" ||
				...
				(entry.type === "custom_message" && SUMMARIZABLE_CUSTOM_TYPES.has(entry.customType)),
		)?.id;
