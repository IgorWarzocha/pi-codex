...
const getProxyForUrl = (require("proxy-from-env") as { getProxyForUrl: GetProxyForUrl }).getProxyForUrl;

let cachedWebSocket: WebSocketConstructorLike | null = null;

async function getWebSocketConstructor(url: string, env?: ProviderEnv): Promise<WebSocketConstructorLike | null> {
	if (process.versions.bun) {
		if (!env && cachedWebSocket) return cachedWebSocket;
	...
	const WebSocketWithProxy = class extends UndiciWebSocket {
		constructor(socketUrl: string, options?: { headers?: Record<string, string> } | string | string[]) {
			const baseOptions =
				Array.isArray(options) || typeof options === "string" ? { protocols: options } : { ...options };
...
function resolveWebSocketProxyForTargetSync(url: string, env?: ProviderEnv): string | undefined {
...
export async function resolveWebSocketProxyForTarget(url: string, env?: ProviderEnv): Promise<string | undefined> {
	return resolveWebSocketProxyForTargetSync(url, env);
}
