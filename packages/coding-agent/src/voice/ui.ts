...
export function realtimeVoiceMessage(
	...
	return {
		customType: route === "delegation" ? REALTIME_DELEGATION_MESSAGE_TYPE : REALTIME_VOICE_MESSAGE_TYPE,
		content:
			route === "delegation"
...
export function registerCodexVoiceRenderer(pi: ExtensionAPI): void {
	...
	pi.registerEntryRenderer<RealtimeVoiceMessageDetails>(REALTIME_DELEGATION_MESSAGE_TYPE, (entry, _options, theme) => {
		const input = typeof entry.data?.input === "string" ? entry.data.input : "Voice delegation unavailable.";
	...
	pi.registerEntryRenderer<RealtimeUserTranscriptMessageDetails>(
		...
		(entry, _options, theme) => {
			const transcript =
				typeof entry.data?.transcript === "string" ? entry.data.transcript : "Voice transcript unavailable.";
			return voiceBox(theme, "You said", transcript);
		},
	);
	pi.registerEntryRenderer<VoiceContextMessageDetails>(VOICE_CONTEXT_MESSAGE_TYPE, (entry, _options, theme) => {
		const summary = typeof entry.data?.summary === "string" ? entry.data.summary : "No voice context summary.";
