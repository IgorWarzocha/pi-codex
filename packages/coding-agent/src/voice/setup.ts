...
export function formatVoiceAudioError(error: Error, mode: CodexVoiceMode, config: CodexConversionConfig): string {
	...
	const setting = input ? "piCodex.voice.inputDevice" : "piCodex.voice.outputDevice";
	...
	return [
		...
		`Agent: inspect the available audio devices and routes. Remove \`${setting}\` in \`${join(getAgentDir(), "settings.json")}\` to use the system default, or set an exact device id. For shared or processed audio, prefer the final virtual/system source rather than opening physical hardware directly. Then ask the user to try using voice features again.`,
...
function audioErrorDirection(message: string, mode: CodexVoiceMode): "input" | "output" | undefined {
	...
	if (
		mode === "dictation" &&
		/(?:requested )?device|audio (?:device|stream)|capture stream|sample format/.test(normalized)
	)
		return "input";
	return undefined;
}
...
export function buildVoiceSetupInstructions(options: {
	...
	return [
		...
		"Set piCodex.voice.audioSetupCompleted to true and preserve every other config value.",
