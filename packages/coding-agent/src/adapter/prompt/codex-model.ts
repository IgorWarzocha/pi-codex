...
export function isCanonicalCodexSubscriptionModel(
	...
	return Boolean(model && model.api === "openai-codex-responses" && isCanonicalCodexBaseUrl(model.baseUrl));
}

export function isCanonicalCodexAliasModel(model: Partial<CodexLikeModelDescriptor> | null | undefined): boolean {
...
export function isCodexTransportModel(model: Partial<CodexLikeModelDescriptor> | null | undefined): boolean {
	return isOpenAICodexModel(model) || isCanonicalCodexSubscriptionModel(model);
}
