...
export function normalizeVoiceContextReasoning(value: unknown): VoiceContextReasoning {
	return typeof value === "string" && (VOICE_CONTEXT_REASONING_LEVELS as readonly string[]).includes(value)
...
export function normalizeCodexConversionConfig(value: unknown): CodexConversionConfig {
	...
	const executionMode =
		normalizeExecutionMode(value["executionMode"]) ?? DEFAULT_CODEX_CONVERSION_CONFIG.executionMode;
	return {
		executionMode,
		...
		scope: {
			allProviders:
				normalizeAllProvidersMode(scope["allProviders"]) ?? DEFAULT_CODEX_CONVERSION_CONFIG.scope["allProviders"],
		...
		ui: {
			...
			toolRenaming: bool(
				ui["toolRenaming"],
				bool(ui["toolRendering"], DEFAULT_CODEX_CONVERSION_CONFIG.ui["toolRenaming"]),
			),
		...
		openai: {
			...
			cacheKeepalive: bool(openai["cacheKeepalive"], DEFAULT_CODEX_CONVERSION_CONFIG.openai["cacheKeepalive"]),
...
export function resolveNativePiCodexConfig(options: {
	settings: unknown;
	executionMode: ExecutionMode;
	notebook: { maxHeapMiB: number; profile?: string | undefined };
}): CodexConversionConfig {
	const settings = isObject(options.settings) ? options.settings : {};
	const tools = {
		customRustBinariesDir: settings["customRustBinariesDir"],
		viewImageFallback: settings["viewImageFallback"],
	};
	return normalizeCodexConversionConfig({
		...settings,
