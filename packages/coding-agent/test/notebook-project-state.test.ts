...
test("project notebook merge preserves a concurrent same-name edit", () => {
	...
	assert.deepEqual(
		merged.entries.map(({ name, kind, hash: entryHash }) => ({ name, kind, entryHash })),
		[
			{
				name: "shared",
				kind: "function",
...
test("project notebook merge treats metadata edits as concurrent changes", () => {
	...
	const merged = mergeProjectState({
		baseline: {
			...
			entries: [{ name: "shared", hash: hash(payload), description: "base" }],
		},
...
function projectCandidate(
	payload: Buffer,
	kind: "value" | "function" = "function",
	metadata: { description?: string; usage?: string } = {},
