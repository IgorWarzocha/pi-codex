...
test("notebook diagnostics group historical duplicates and put runtime health first", () => {
	const diagnostics: NotebookDiagnostic[] = [
		diagnostic("cell-1", 0),
		diagnostic("cell-2", 1),
		diagnostic("cell-3", 2, "other"),
		diagnostic("cell-4", 3, "r", "error", "ts"),
	];
	...
	assert.deepEqual(
		(
			result.details as {
				diagnosticGroups: Array<{ count: number; name?: string; severity: string; source?: string }>;
			}
		).diagnosticGroups.map(({ count, name, severity, source }) => ({ count, name, severity, source })),
		[
...
test("notebook reset preserves the durable project manifest and pins", async () => {
	...
	const manifest: ProjectStateManifest = {
		...
		entries: [
			{ name: "prReview", kind: "value", offset: 0, length: payload.length, hash: hash(payload), pinned: true },
		],
		skipped: [],
	};
	...
	writeFileSync(
		join(paths.directory, "npm-imports.json"),
		`${JSON.stringify({ schema: 1, project, imports: ["npm:example@1.2.3"] })}\n`,
	);
	...
	try {
		...
		assert.deepEqual(JSON.parse(readFileSync(join(paths.directory, "npm-imports.json"), "utf8")).imports, [
			"npm:example@1.2.3",
		]);
...
test("terminating a notebook cell invalidates its kernel before another cell can use it", async () => {
	assert.equal(
		isNotebookBootstrapFailure({ errorText: "TypeError: Cannot read properties of undefined (reading 'begin')" }),
		false,
	);
	assert.equal(
		isNotebookBootstrapFailure({ errorText: "Notebook runtime bootstrap unavailable: __piNotebook.begin" }),
		true,
	);
	...
	const pending = new Promise<FakeResult>((resolve) => {
		finish = resolve;
	});
	kernel = {
		execute: () => {
	...
	const quickSession = {
		...
		kernel: () => quickKernel,
		invalidateKernel: async () => {
			quickKernel = undefined;
		},
	};
	const quickRuntime = new NotebookExecutionRuntime(
		() => quickSession as never,
		async () => {},
	);
	const quickYielded = await quickRuntime.execute(
...
function diagnostic(
	...
	cellIndex: number,
	name = "r",
	severity: NotebookDiagnostic["severity"] = "warning",
	source = "deno",
): NotebookDiagnostic {
	return {
		cellId,
