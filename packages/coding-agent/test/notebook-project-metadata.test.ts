...
} from "../src/tools/notebook-mode/project-state-runtime.ts";
...
test("durable binding metadata ignores getters, survives restore, and is shown by status", async () => {
	...
	try {
		const captureSource = projectStateCaptureSource({
			candidates: ["probe"],
			payloadPath: join(root, "candidate.bin"),
			manifestPath: join(root, "candidate.json"),
			maxBytes: 8 * 1024 * 1024,
		}).replace('const { serialize } = await import("node:v8");', "");
		const capture = new Function("Deno", "probe", "serialize", `return (async () => ${captureSource})()`);
		await capture(captureDeno, helper, serialize);
		...
		try {
			...
			const restoreSource = projectStateRestoreSource(
				{
					deno: "2.9.5",
					v8: "test",
			...
			).replace('const { deserialize, serialize } = await import("node:v8");', "");
			const restore = new Function(
				"Deno",
				"crypto",
				"deserialize",
				"serialize",
				`return (async () => ${restoreSource})()`,
			);
			await restore(
				{
					version: { deno: "2.9.5", v8: "test" },
