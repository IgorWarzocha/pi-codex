...
test("session checkpoints retain their project baseline without trusting old cleanup paths", async () => {
	...
	const key = createHash("sha256")
		...
		.digest("hex");
	const directory = join(agentDir, "cache", "pi-codex", "notebook-mode", "sessions", key);
	...
	const execute = async (source: string) => {
		...
		const run = new Function(
			...
			"crypto",
			"serialize",
			`return (async () => ${source.replace('const { serialize } = await import("node:v8");', "")})()`,
		);
		await run(deno, { randomUUID }, serialize);
