...
import { promiseWithResolvers } from "../src/voice/promise-resolvers.ts";
...
test("LAN browser preserves handoff and restarts after explicit release", async () => {
	...
	assert.deepEqual(
		...
		[{ type: "connected" }, { type: "active", mode: "conversation", muted: false }],
	);
...
test("LAN browser takeover shares an in-progress host conversation setup", async () => {
	const setup = promiseWithResolvers<void>();
	let hostStarts = 0;
