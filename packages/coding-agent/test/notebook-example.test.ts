...
test("Notebook seeds expose a self-describing foo/bar example and skip conflicts", async () => {
	...
	try {
		...
		const foo = (globalThis as Record<string, unknown>)["foo"] as { description: string; usage: string }[] & {
			description: string;
			usage: string;
		};
		const bar = (globalThis as Record<string, unknown>)["bar"] as { description: string; usage: string } & ((
			item: unknown,
		) => unknown);
