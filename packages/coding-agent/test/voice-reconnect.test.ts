...
import type { CodexRealtimePeerEvent, CodexRealtimeWebRtcPeer } from "../src/voice/conversation/peer.ts";
import { type CodexConversationCallbacks, CodexRealtimeConversation } from "../src/voice/conversation/session.ts";
...
test("only an established realtime transport failure is a resumable drop", async () => {
	...
	await startup.session.start(AUTH, DEFAULT_CODEX_CONVERSION_CONFIG, "instructions");
	...
	await active.session.start(AUTH, DEFAULT_CODEX_CONVERSION_CONFIG, "instructions");
