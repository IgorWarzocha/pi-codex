...
test("Notebook exec proxy shares control normalization without changing the prompt", async () => {
	...
	} as Pick<CodeModeRuntime, "controlNotebook"> as CodeModeRuntime;
	...
	const modes = new CodeModeRuntime({
		agentDir: "/tmp",
		cwd: "/project",
		getTools: () => [],
		getNotebookOptions: () => ({ agentDir: "/tmp", maxHeapMiB: 4096 }),
	});

	assert.equal(proxy.deferLoading, true);
	assert.equal(buildCodeModeToolsPrompt([proxy]), "");
	modes.setExecutionKind("code");
	assert.deepEqual(modes.collectTools(), []);
	modes.setExecutionKind("notebook");
	assert.equal(modes.collectTools().at(-1)?.name, "notebook");
	for (const request of [
		{ action: "status" },
		{ action: "list", query: "saved*" },
		{ action: "diagnostics" },
	] as const) {
		assert.deepEqual(await proxy.invoke(request, context, controller.signal), {
			message: `Ran ${request.action}`,
			details: { action: request.action },
		});
	}
	assert.deepEqual(
		calls.map(({ request }) => request),
		[{ action: "status" }, { action: "list", query: "saved*" }, { action: "diagnostics" }],
	);
	for (const request of [
		...
		const normalized = request.action === "pin" ? { action: "pin" as const, names: ["alpha"] } : request;
		assert.deepEqual(await proxy.invoke(request, context, controller.signal), {
	...
	assert.equal(
		calls.every((call) => call.context === context && call.signal === controller.signal),
		true,
	);
