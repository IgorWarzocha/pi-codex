...
test("native compaction excludes voice-only chatter but preserves Pi delegations", () => {
	const customEntry = (id: string, parentId: string | null, customType: string, content: string) => ({
		type: "custom_message",
		id,
		parentId,
		timestamp: new Date(1).toISOString(),
	...
	const chatter = customEntry("chatter", null, REALTIME_VOICE_MESSAGE_TYPE, "voice-only conversation");
	const delegation = customEntry("delegation", "chatter", REALTIME_DELEGATION_MESSAGE_TYPE, "Pi-visible delegation");
...
test("injects pending native compacted window into Pi compaction summarization payload", async () => {
	...
	assert.deepEqual(
		...
		["developer", "compaction_summary", "user"],
	);
