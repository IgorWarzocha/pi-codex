...
Definitions remain live for execution, but the custom-tool prompt is snapshotted when a session starts. A promoted tool added or renamed during a session remains deferred until compaction, reload, or a new, resumed, or forked session. When Pi discovers it, Pi appends its usage as a non-triggering developer capability update for the current session. Compaction refreshes custom-tool promotion for subsequent turns; later sessions include it in their initial system prompt.
...
To enable one, copy its top-level TOML and matching companion directory into `custom-tools/`, preserving their relative layout. Examples are references, not defaults; never copy or enable one merely because it exists.
